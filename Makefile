...
composer-install:
	@docker run --rm -v $(shell pwd):/app composer install --ignore-platform-reqs
...
gen-certs:
	@docker run --rm -v $(shell pwd)/docker/ssl:/certificates -e "SERVER=$(NGINX_HOST)" jacoelho/generate-certificate
...
resetOwner:
	@$(shell chown -Rf $(SUDO_USER):$(shell id -g -n $(SUDO_USER)) $(MYSQL_DUMPS_DIR) "$(shell pwd)/docker/ssl" "$(shell pwd)/web/app" 2> /dev/null)
